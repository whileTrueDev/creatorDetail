...
const UpdateFollower = ({ creatorId, connection, accessToken }) => new Promise((resolve, reject) => {
...
const dividedGetFollower = (targetList) => new Promise((resolve, reject) => {
  pool.getConnection(async (err, connection) => {
    if (err) {
      ...
      Promise.all(
        targetList.map((creatorId) => UpdateFollower({ creatorId, connection, accessToken }))
      ).then(() => {
        connection.release();
        setTimeout(() => {
          resolve();
